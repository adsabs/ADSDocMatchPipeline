...
from adsdocmatch.authors import normalize_author_list

proj_home = os.path.realpath(os.path.join(os.path.dirname(__file__), '../'))
conf = load_config(proj_home=proj_home)

logger = setup_logging('docmatch_log_from_oracle', level=conf.get('LOGGING_LEVEL','WARN'), proj_home=proj_home, attach_stdout=conf.get('LOG_STDOUT','FALSE'))
...
def get_matches(metadata, doctype, mustmatch=False, match_doctype=None):
    ...
    sleep_sec = int(conf.get('API_DOCMATCHING_ORACLE_SERVICE_SLEEP_SEC', 30))
    try:
        num_attempts = int(conf.get('API_DOCMATCHING_ORACLE_SERVICE_ATTEMPTS', 10))
        for i in range(num_attempts):
            response = requests.post(
                url=conf.get('API_DOCMATCHING_ORACLE_SERVICE_URL', None) + '/docmatch',
                headers={'Authorization': 'Bearer %s' % conf.get('API_DOCMATCHING_TOKEN', None)},
                data=json.dumps(payload),
                timeout=60
            )
            status_code = response.status_code
            if status_code == 200:
                logger.debug('Got 200 for status_code at attempt # %d' % (i+1))
                break
            # if got 5xx errors from solr, per alberto, sleep for five seconds and try again, attempt 3 times
            elif status_code in [502, 504]:
                logger.warning('Got %d status_code from solr, waiting %d second and attempt again.' % (status_code, num_attempts))
                time.sleep(sleep_sec)
            # any other error, quit
            else:
                logger.error('Got %s status_code from a call to oracle, stopping.' % status_code)
                break
    except Exception as e:
        status_code = 500
        logger.warning('Exception %s, stopping.'%str(e))
